...
public interface DiagnosesRepository extends CrudRepository<Diagnosis, Long> {
    @Query(value = "SELECT * FROM diagnosis d LEFT JOIN users_diagnoses us on d.id = us.diagnoses_id LEFT JOIN users u on us.patient_user_id = u.user_id WHERE u.user_id = :id", nativeQuery = true)
